...
class TestOfferDeleteArchive:
    def test_offer_delete_archives_via_status(self, runner):
        with patch("requests.patch") as mock_patch:
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            mock_resp.content = b'{"updated":1}'
            mock_resp.json.return_value = {"updated": 1}
            mock_patch.return_value = mock_resp
            result = runner.invoke(cli, ["offer", "delete", "456", "--confirm"])
            assert result.exit_code == 0
            _, kwargs = mock_patch.call_args
            assert kwargs["json"]["status"] == "archived"
            assert "456" in kwargs["json"]["ids"]


class TestUpdateStatusChoice:
    def test_campaign_update_rejects_invalid_status(self, runner):
        result = runner.invoke(cli, [
            "campaign", "update", "123", "--status", "deleted"
        ])
        assert result.exit_code != 0

    def test_offer_update_rejects_invalid_status(self, runner):
        result = runner.invoke(cli, [
            "offer", "update", "456", "--status", "deleted"
        ])
        assert result.exit_code != 0
